...
void setup() {
  ...
  serveur.on("/", HTTP_GET, [](AsyncWebServerRequest *request){
    request->send(SPIFFS, "/radiateur.html", "text/html");
  });
  serveur.on("/radiateur.css", HTTP_GET, [](AsyncWebServerRequest *request){
    request->send(SPIFFS, "/radiateur.css", "text/css");
  });
  serveur.on("/radiateur.js", HTTP_GET, [](AsyncWebServerRequest *request){
    request->send(SPIFFS, "/radiateur.js", "text/javascript");
  });
...
void loop() {
  ...
  if(reveil_active_serveur){//Si éteindre lampe soir est à "Oui" sur le site internet
    if (heure==heure_lampe_soir_serveur && minute==minute_lampe_soir_serveur){//regarder si l'heure actuelle est supérieure ou égale à l'heure choisie sur le site
      if(weekend_active_reveil_serveur){//Si dans Lampe soir week-end est à oui sur le serveur
        eteidre_lum();
      }
